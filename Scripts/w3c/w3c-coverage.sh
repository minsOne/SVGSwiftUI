...
if [[ -z "$MANIFEST" || -z "$OUTPUT" ]]; then
  cat <<USAGE
Usage: ./Scripts/w3c/w3c-coverage.sh <manifest-path> <output-path> [--strict]

Examples:
  ./Scripts/w3c/w3c-coverage.sh Tests/SVGSwiftUITests/W3C/w3c-manifest.json docs/W3C_COVERAGE.md
...
lines = []
lines.append("# W3C Conformance Coverage\n")
...
lines.append("| ID | Suite | Category | Mode | Expected | SVG | Reference | SVG Resource | Reference Resource |\n")
lines.append("|---|---|---|---|---|---|---|---|---|\n")
for item in entries:
    ...
    lines.append(
        ...
        f"{item['expected']} | `{item['svg']}` | `{item['reference']}` | {svg_mark} | {ref_mark} |\n"
    )

lines.append("\n")
lines.append("- pass/fail/unsupported are derived from manifest `expected` field.\n")
lines.append("- Unsupported coverage should be reviewed against runtime parser capability after implementation progress.\n")
...
if strict_mode and has_strict_error:
    raise SystemExit(1)
PY
